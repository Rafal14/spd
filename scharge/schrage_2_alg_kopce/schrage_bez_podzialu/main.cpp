...
/*
*
*  Realizacja algorytmu Schrage bez przedzialu
*
*
*/
...
int main()
{
   ...
   //zapisywanie danych
   fstream plikWyj;
   plikWyj.open("wyniki.txt", fstream::out);
   ...
   for ( int i=0; i < 9; ++i )
   {
	...
	plikWyj << "permutacja:" << endl;
	for ( unsigned int licz=0; licz < tabN.size(); ++licz )
	{
		plikWyj << tabG.at(licz).j << endl; 
	}
    //usuniecie
    while (!tabN.empty()) {
       tabN.pop_back();
